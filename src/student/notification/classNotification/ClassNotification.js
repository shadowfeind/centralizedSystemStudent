...
import {
  getAllClassNotificationStudentAction,
  getListClassNotificationStudentAction,
} from "./ClassNotificationActions";
import {
  GET_ALL_CLASS_NOTIFICATION_STUDENT_RESET,
  GET_LIST_CLASS_NOTIFICATION_STUDENT_RESET,
...
const ClassNotification = () => {
  ...
  const {
    ...
    loading,
    error: listClassNotificationStudentError,
  ...
  useEffect(() => {
    ...
    if (classNotificationStudent) {
      ...
      setAcademicYearValue(
        classNotificationStudent?.searchFilterModel.ddlAcademicYear[0]?.Key
      );
      setShift(classNotificationStudent?.searchFilterModel.ddlAcademicShift);
      setShiftValue(
        classNotificationStudent?.searchFilterModel.ddlAcademicShift[0].Key
      );
      setProgramValue(
        classNotificationStudent?.searchFilterModel.ddlFacultyProgramLink[0].Key
      );
      setSection(classNotificationStudent?.searchFilterModel.ddlSection);
      setSectionValue(
        classNotificationStudent?.searchFilterModel.ddlSection[0].Key
      );
      setClassOpt(classNotificationStudent?.searchFilterModel.ddlClass);
      setClassOptValue(
        classNotificationStudent?.searchFilterModel.ddlClass[0].Key
      );
    }
            ...
            <TableContainer className={classes.table}>
              ...
              <TableBody>
                {tableDataAfterPagingAndSorting().map((item) => (
                  <ClassNotificationStudentTableCollapse
                    item={item}
                    key={item.$id}
                  />
                ))}
